...
import { applySpineSchema, resetPublicSchema } from "./migrate.ts";
import { createPostgresStore, insertUser } from "./postgres.ts";
...
describePersistence("postgres persistence + RLS", () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query("GRANT ndapp_authenticated TO CURRENT_USER");
  });

  it("round-trips income and allocation through the domain engine", async () => {
    const alice = crypto.randomUUID();
    const bob = crypto.randomUUID();
    await insertUser(pool, { id: alice, email: "alice@example.com", passwordHash: "x" });
    await insertUser(pool, { id: bob, email: "bob@example.com", passwordHash: "x" });
  ...
  it("RLS: member A cannot read household B; writes as authenticated are denied", async () => {
    ...
    await insertUser(pool, { id: alice, email: "alice@example.com", passwordHash: "x" });
    await insertUser(pool, { id: carol, email: "carol@example.com", passwordHash: "x" });
    const app = createBudgetApp(createPostgresStore(pool));
    ...
    try {
      ...
      await client.query("SET LOCAL ROLE ndapp_authenticated");
      ...
      const stolen = await client.query("SELECT * FROM budget_months WHERE household_id = $1", [
        // carol's household is unknown to this query on purpose; load all
        houseA.householdId,
      ]);
      expect(stolen.rowCount).toBe(1);
